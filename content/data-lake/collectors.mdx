...
Cardinal Data Lake ingests telemetry from [OpenTelemetry Collectors](https://opentelemetry.io/docs/collector/) that write data to your S3-compatible object storage bucket. This guide covers the recommended three-tier collector architecture for monitoring Kubernetes clusters.
...
| `LAKERUNNER_ORGANIZATION_ID` | gateway/deployment.yaml | The organization UUID provisioned during Cardinal Data Lake installation. This is the same value generated by the `Organization ID` field in the [Manual install](/data-lake/manual-install) wizard. Telemetry is written to `otel-raw/{LAKERUNNER_ORGANIZATION_ID}/{K8S_CLUSTER_NAME}/`, so the value here must match the org ID configured in your Cardinal Data Lake deployment. |
...
If you already run [Grafana Alloy](https://grafana.com/docs/alloy/) in your cluster, Alloy can write directly to the same S3 bucket and object layout that Cardinal Data Lake ingests. This lets you skip the Cardinal gateway entirely and use Alloy as your sole collector.

### How it works

Cardinal Data Lake's S3 ingest is format-driven, not source-driven. Any process that writes OTLP-protobuf files to the right prefix, with the right filename shape, gets picked up:
...
The snippet below receives OTLP from workloads, stamps the cluster name as a resource attribute, enriches with Kubernetes metadata, converts cumulative metrics to delta, and writes to S3 in the Cardinal Data Lake-compatible layout. Substitute your own values for the four environment variables.
...
//   LAKERUNNER_ORGANIZATION_ID  — the same UUID as your Cardinal Data Lake install
...
Objects land at keys like `otel-raw/<org>/<cluster>/year=.../minute=.../logs_<rand>.binpb.gz` (and `metrics_`, `traces_`), which matches what Cardinal Data Lake's pubsub handler expects.
...
- **Cumulative-to-delta is essential for external Prometheus-style counters.** Cardinal Data Lake stores temporality as-is, so cumulative counters without conversion will produce confusing `rate()` results across collector restarts. The `otelcol.processor.cumulativetodelta` step in the snippet above handles this — don't remove it.
