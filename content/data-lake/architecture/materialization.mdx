...
Cardinal Data Lake can selectively convert logs and traces into metric series at ingest time. You define rules for the conversions you care about, and Cardinal Data Lake evaluates those rules during ingest, emitting derived metrics (counts, rates, histograms, aggregates) as first-class metric segments. These derived metrics flow through the same compaction, rollup, and query pipeline as any native metric.
...
During ingest, Cardinal Data Lake evaluates configured rules against incoming data and emits materialized metric rows in 10s buckets. These rows include standard rollup fields (`chq_rollup_count`, `chq_rollup_sum`, sketches), so they are immediately compatible with the existing metrics pipeline.
...
At query time, Cardinal Data Lake analyzes query expressions and rewrites only sub-expressions that are provably compatible with materialized rules. The rest of the query runs normally against raw data.

## Correctness Model

Rewrite is guarded, not best-effort. Cardinal Data Lake only rewrites when semantics are preserved:
...
If any guard fails, Cardinal Data Lake falls back to the raw path automatically.

## How Cardinal Data Lake Is Different
...
Cardinal Data Lake eliminates this trade-off:
