...
With `--host` set, nothing touches Cardinal Cloud: your Maestro runs the consent flow, mints the keys, and hands the plugin **your** endpoints — MCP on your Maestro, telemetry to your Lakerunner intake. The telemetry side requires your operator to have set `MAESTRO_INGEST_ENDPOINT` on the Maestro deployment (see [Environment variables](/maestro/install/environment)); without it the connect bundle omits telemetry and sessions never reach the dashboard.
