...
| `MAESTRO_INGEST_ENDPOINT` | For Agent Outcomes | — | Absolute OTLP/HTTP endpoint of **your** Lakerunner intake (e.g. `https://intake.example.internal`). Returned in the `cardinal-connect` bundle so the Claude Code / Codex plugins send session telemetry to your Lakerunner instead of Cardinal Cloud. When unset, connect bundles omit the telemetry side entirely and the [Agent Outcomes dashboard](/maestro/agent-outcomes) stays empty |
