...
If you only want Proxmox's own node, guest, and storage stats and are on Proxmox VE 9, there is a far shorter path with nothing to install: [Exporting Proxmox VE built-in metrics over OTLP](/how-to/proxmox-otlp-metric-server). The two guides are complementary — run both if you want PVE's guest-level stats *and* Ceph detail plus logs.
...
<Prerequisites
  items={[
    ...
    },
    {
      ...
      alt: <>Network path from the PVE management network to your collector gateway / Cardinal Data Lake / vendor OTLP endpoint, on gRPC <code>:4317</code> (or HTTP <code>:4318</code>).</>,
    },
  ]}
/>
...
| `<your-cluster-name>` | A stable identifier for this Ceph / Proxmox cluster. Stamped onto every record as `proxmox.cluster.name`; downstream consumers (Cardinal Data Lake, dashboards, alerts) use it to partition by source. |
...
<NextSteps
  items={[
    {
      icon: '🖥',
      title: 'Proxmox built-in metrics',
      description: "Add PVE's own node, guest, and storage stats with the VE 9 OpenTelemetry metric server.",
      href: '/how-to/proxmox-otlp-metric-server',
    },
    {
      icon: '📦',
