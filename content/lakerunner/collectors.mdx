...
| `LAKERUNNER_ORGANIZATION_ID` | gateway/deployment.yaml | The organization UUID provisioned during Lakerunner installation. This is the same value generated by the `Organization ID` field in the [Manual install](/lakerunner/manual-install) wizard. Telemetry is written to `otel-raw/{LAKERUNNER_ORGANIZATION_ID}/{K8S_CLUSTER_NAME}/`, so the value here must match the org ID configured in your Lakerunner deployment. |
