...
Version **0.2.0** supports the OpenCode 1.x server-plugin API (tested with 1.18.30); OpenCode V2 is not supported. You need **Node.js 20+**, **Python 3.9+** on your `PATH`, and **macOS or Linux**.
...
npm install --prefix "$HOME/.local/share/cardinal" \
  https://github.com/cardinalhq/cardinal-agent-plugins/releases/download/opencode-v0.2.0/cardinalhq-opencode-plugin-0.2.0.tgz
```
...
```bash
"$HOME/.local/share/cardinal/node_modules/.bin/cardinal-opencode" decision on
```

While it's on, the plugin gives chat turns recording instructions plus the session's decisions so far, and the agent records choices with the `cardinal_record_decision` tool. Each decision is sent tagged with the session, repo, branch, and PR. `decision off` turns it off and `decision status` shows the current state. The OpenCode hooks this uses are marked experimental in OpenCode 1.x.
