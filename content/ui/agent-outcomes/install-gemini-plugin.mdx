...
- Gemini CLI 0.26.0 or later, where hooks are on by default. Earlier versions need hooks explicitly enabled in Gemini CLI's settings.
...
| `~/.gemini/extensions/cardinal/hooks/hooks.json` | Managed Cardinal hook entries for `SessionStart`, `BeforeAgent`, `AfterModel`, `AfterTool`, `PreCompress`, and `SessionEnd`. |
...
## Upgrading from before 0.17.0

Plugin versions before 0.17.0 wrote a hook registration that Gemini CLI ignored, so their hooks never ran. Re-run `cardinal-connect` to rewrite it (no new consent needed), then restart Gemini CLI. `cardinal-status` reports an old registration or a Gemini CLI version too old to load hooks.

## Pull requests and decisions

Each prompt's branch state includes the branch's pull request when `gh` is installed and signed in and a PR exists. Network work runs in the background, so the hooks don't slow your prompts.

**Decision capture** is off by default:

```bash
python3 scripts/cardinal-decision on
```

While it's on, each prompt tells Gemini to record material choices with `cardinal-decision record …`, and each decision is sent tagged with the session, repo, branch, and PR. `python3 scripts/cardinal-decision off` turns it off. With Gemini CLI's sandbox enabled, the record command can't write `~/.gemini`; it exits with a message telling you to allow that path or run without the sandbox.

## Privacy

The plugin captures tool names, bash command lines, and file paths so Cardinal can tell which repo and service a session worked on. The contents of your prompts are never captured. Per-turn token counts come straight from Gemini CLI's `AfterModel` hook payload — no transcript scraping — and are attributed to the model id the hook reports. Hooks don't receive cached-token counts, so reported cost is an upper bound.

## Payload-shape capture

The hooks follow Gemini CLI's published hook payload types, but haven't been observed widely in the wild. If your sessions look sparse, set `CARDINAL_GEMINI_DEBUG_PAYLOADS=1` before starting Gemini CLI — raw hook payloads land under `~/.gemini/cardinal/telemetry/debug/<Event>-<ts>.json`. Share those with the plugin maintainers so the parity spec can be locked to real key names.
