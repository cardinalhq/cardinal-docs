...
The Agent Outcomes dashboard answers two questions about your org's coding-agent spend: **what did it cost, and did the work ship?** Every Claude Code session run with the [`cardinal-claude-plugin`](https://github.com/cardinalhq/cardinal-claude-plugin), every Codex CLI session run with the [`cardinal-codex-plugin`](https://github.com/cardinalhq/cardinal-codex-plugin), every Cursor session run with the [`cardinal-cursor-plugin`](https://github.com/cardinalhq/cardinal-cursor-plugin), every Gemini CLI session run with the [`cardinal-gemini-plugin`](https://github.com/cardinalhq/cardinal-gemini-plugin), every OpenCode or Pi session run with Cardinal's [OpenCode plugin](/ui/agent-outcomes/install-opencode-plugin) or [Pi extension](/ui/agent-outcomes/install-pi-plugin), and every Devin session picked up by the [Devin poller](/ui/agent-outcomes/install-devin-poller) is attributed to an engineer, a git branch, a pull request, and an **initiative** (one branch = one initiative), then classified by outcome: **merged**, **in flight**, **lost** (closed or gone stale without merging), or **ad hoc** (research and diagnostic work that was never headed to a PR).

To get your sessions onto the dashboard, install the plugin for your agent:

- [Install: Claude Code plugin](/ui/agent-outcomes/install-claude-plugin)
- [Install: Codex CLI plugin](/ui/agent-outcomes/install-codex-plugin)
- [Install: Cursor plugin](/ui/agent-outcomes/install-cursor-plugin)
- [Install: Gemini CLI plugin](/ui/agent-outcomes/install-gemini-plugin)
- [Install: OpenCode plugin](/ui/agent-outcomes/install-opencode-plugin)
- [Install: Pi extension](/ui/agent-outcomes/install-pi-plugin)
- [Install: Devin poller](/ui/agent-outcomes/install-devin-poller)

Every plugin also records the branch's pull request on each session when the GitHub CLI (`gh`) can resolve it, and offers opt-in **decision capture**: while it's on, the agent records the choices it makes (which approach, which open question it settled) so they show up alongside the session. Each install page explains how to turn it on. Devin runs in Cognition's cloud, so instead of a plugin, the Devin poller reads your org's sessions through the Devin API: it takes each session's pull request from Devin, and captures decisions from sessions started with Cardinal's decision playbook.

It works the same on both deployments:

- **Cardinal Cloud (SaaS)** — `app.cardinalhq.io`, under the Outcomes dashboard.
- **Self-hosted Cardinal UI** — the same dashboard on your Cardinal UI host; session telemetry flows through your own Cardinal Data Lake.
...
- The Cardinal plugin installed in engineers' agent environments — [`cardinal-claude-plugin`](/ui/agent-outcomes/install-claude-plugin) for Claude Code, [`cardinal-codex-plugin`](/ui/agent-outcomes/install-codex-plugin) for Codex CLI, [`cardinal-cursor-plugin`](/ui/agent-outcomes/install-cursor-plugin) for Cursor, [`cardinal-gemini-plugin`](/ui/agent-outcomes/install-gemini-plugin) for Gemini CLI, the [OpenCode plugin](/ui/agent-outcomes/install-opencode-plugin), or the [Pi extension](/ui/agent-outcomes/install-pi-plugin). The plugin attributes sessions to branches and initiatives; branch names following `<type>/<kebab-name>` (`feat/`, `fix/`, `refactor/`, `infra/`, `research/`) classify the initiative type.
- A connected **GitHub** integration, for PR outcome classification (merged / open / closed).
- **Cardinal Data Lake**, which stores the session telemetry the dashboard reads.
