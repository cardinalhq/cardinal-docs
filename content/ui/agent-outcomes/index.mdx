...
- [Install: omnigent policy](/ui/agent-outcomes/install-omnigent-policy)
...
- The Cardinal plugin installed in engineers' agent environments — [`cardinal-claude-plugin`](/ui/agent-outcomes/install-claude-plugin) for Claude Code, [`cardinal-codex-plugin`](/ui/agent-outcomes/install-codex-plugin) for Codex CLI, [`cardinal-cursor-plugin`](/ui/agent-outcomes/install-cursor-plugin) for Cursor, or [`cardinal-omnigent-policy`](/ui/agent-outcomes/install-omnigent-policy) loaded into an omnigent server. The plugin attributes sessions to branches and initiatives; branch names following `<type>/<kebab-name>` (`feat/`, `fix/`, `refactor/`, `infra/`, `research/`) classify the initiative type.
