...
Browse to `MAESTRO_BASE_URL`. You'll be redirected to your IdP, then back to Cardinal UI. If your email is in `OIDC_SUPERADMIN_EMAILS` (or you're in `OIDC_SUPERADMIN_GROUP`), you'll see an **Admin** section in the sidebar. If not, jump back to [OIDC setup](/ui/install/oidc#bootstrapping-the-first-superadmin).
...
2. Pick a provider:
   - **Bedrock** — see the [Bedrock guide](/ui/install/bedrock). Leave keys blank if using IRSA
...
Then open **Admin → LLM Model Catalog** and select which specific models to expose. Cardinal UI won't surface a model to org users until you've mapped it here.

## 3. Create your first organization

Everything in Cardinal UI is scoped to an org. Users, integrations, chats, tokens — all of it.
...
- [Cardinal Data Lake](/ui/integrations/lakerunner) — almost certainly first
- [PostgreSQL](/ui/integrations/postgres), [Snowflake](/ui/integrations/snowflake), [BigQuery](/ui/integrations/bigquery), etc. — data sources
- [GitHub](/ui/integrations/github), [ServiceNow](/ui/integrations/servicenow) — actions
- [Slack](/ui/integrations/slack), [Teams](/ui/integrations/teams), [PagerDuty](/ui/integrations/pagerduty) — notifications
...
For outbound notifications (alerts, daily summaries) use **Admin → Notification Integrations**. This is separate from per-org integrations — it's where you wire a Slack workspace or Teams channel that Cardinal UI itself posts to for system events.
...
1. Plan a step that calls a Cardinal Data Lake tool
2. Execute it via the MCP Gateway
3. Return the logs

If it refuses or errors, check **Admin → Usage** (token usage proves LLM calls are going through) and the Cardinal UI pod logs.
...
The chart already wires this for both liveness and readiness. If you put a separate uptime monitor in front of Cardinal UI, this is the endpoint to hit — it doesn't require auth.

For operational visibility, **Admin → System Health** surfaces per-org Cardinal Data Lake instance health and recent errors.
...
- **Scale**: `maestro.replicas` can go above 1 only if you've front-ended Cardinal UI with a load balancer that does session affinity. The current session storage is per-pod. Plan to revisit if you need HA.
