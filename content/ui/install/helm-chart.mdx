...
Every field in `values.yaml` for the `maestro` Helm chart. Defaults shown are from the chart (`charts/ui/values.yaml`).
...
| `maestro.replicas` | `1` | Cardinal UI is stateless but session cookies are not sticky-bound — keep at `1` unless you've set up shared session storage |
...
Starting with chart **0.8.0**, the MCP gateway runs as a Kubernetes native sidecar (initContainer with `restartPolicy: Always`) inside every Cardinal UI pod and every github-cache pod. There is no standalone Deployment or Service by default — Cardinal UI talks to it over `localhost`, so every MCP session stays pod-local. The `mcpGateway.enabled` and `mcpGateway.replicas` fields from older chart versions no longer exist.
...
| Field | Default | Notes |
| ----- | ------- | ----- |
| `mcpGateway.port` | `8080` | Sidecar listen port. Cardinal UI reaches it at `http://localhost:8080`. |
...
| `mcpGateway.service.enabled` | `false` | When `true`, creates `<release>-mcp-gateway` ClusterIP Service targeting the sidecar's port. **Only useful for single-pod installs**; for any multi-replica deployment, expose the MCP entry point through Cardinal UI instead (`MAESTRO_MCP_API_KEY` on the Cardinal UI container — see [Connect AI Clients](/ui/mcp-clients)). |
| `mcpGateway.env` | `[]` | Extra env for the sidecar (e.g. `AWS_REGION` for Bedrock embeddings). |

Leave `apiKey` and `apiKeySecret.name` both empty for most installs — the sidecar runs unauthenticated on `localhost`, and external auth is enforced at the Cardinal UI entry point via `MAESTRO_MCP_API_KEY`. The `mcpGateway.apiKey*` knobs only matter if you also flipped `service.enabled: true` to expose the sidecar directly (a single-pod-only path; in multi-replica installs, route external MCP traffic through Cardinal UI per [Connect AI Clients](/ui/mcp-clients)).

## Database

Required. Cardinal UI will not start without it.
...
| `database.port` | `5432` | |
| `database.name` | `maestro` | Database name (must exist; Cardinal UI does not create it) |
...
The built-in Ingress is a minimal `networking.k8s.io/v1` object that points at the Cardinal UI service. It works for NGINX, Traefik, or anything that honors `ingressClassName`. If you need more control (IngressRoute, Gateway API, multiple hosts), disable this and create your own routing resource — the Cardinal UI service is `<release>-maestro` on port `4200`.
...
| `serviceAccount.annotations` | `{}` | **Add IRSA annotation here for Bedrock.** See [AWS Bedrock](/ui/install/bedrock) |
...
| `global.env` | `[]` | Env vars that apply to both Cardinal UI and the gateway. Use for `AWS_REGION`, tracing config, etc. |
