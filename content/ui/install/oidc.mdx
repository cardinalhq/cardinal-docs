...
Cardinal UI delegates all authentication to an external OIDC provider. There is no built-in password login. Every user who signs in must exist in your IdP first.

## How Cardinal UI uses OIDC

- The browser performs the standard Authorization Code flow against your IdP.
- Cardinal UI validates the resulting ID token against the provider's JWKS.
- The token's `email` claim identifies the user. `email_verified` must be `true`.
- The token's `sub` claim is the persistent **account key** — it's what Cardinal UI stores to recognise a returning user across email changes. Both the email and account-key claim names are remappable (see [Claim mapping](#claim-mapping)) for IdPs whose token shape differs from the Keycloak defaults.
...
| `OIDC_AUDIENCE` | The `aud` claim Cardinal UI expects on incoming tokens. Defaults to `maestro-ui` |
...
| `OIDC_EXTERNAL_ID_CLAIM` | Token claim Cardinal UI uses as the persistent account key. Default `sub`. **Changing this on an existing deployment is a user migration event** — see [Claim mapping](#claim-mapping) |
...
```
https://ui.example.com/api/auth/callback
```

## Required token claims

Cardinal UI reads these claims out of the ID token. Configure your IdP to include them (or, if the claim names differ, remap them with the `OIDC_*_CLAIM` env vars — see [Claim mapping](#claim-mapping)):
...
2. Cardinal UI looks at their `email` (and `groups`, if any).
...
Cardinal UI is developed against Keycloak and the docker-compose dev stack uses it as the default IdP.
...
2. Create a client:
   ...
   - Standard flow: **on**
   - Valid redirect URIs: `https://ui.example.com/*`
...
2. Sign-in redirect URI: `https://ui.example.com/api/auth/callback`.
...
5. Set:
   ...
   If your Okta tenant uses a **custom authorization server** whose audience is a resource URI (e.g. `api://ui`) rather than the app's client ID, split the two:

   ```yaml
   - name: OIDC_AUDIENCE
     value: api://ui            # the `aud` on issued tokens (the auth server's audience)
   - name: OIDC_CLIENT_ID
...
Tell Cardinal UI where to find each logical claim:
...
* `OIDC_EXTERNAL_ID_CLAIM` must not resolve to the same token claim as `OIDC_EMAIL_CLAIM`. Cardinal UI refuses to boot if they do — account keys must be stable, and emails aren't. The literal case (`OIDC_EXTERNAL_ID_CLAIM=email`) and the subtle case (`OIDC_EMAIL_CLAIM=sub` + `OIDC_EXTERNAL_ID_CLAIM=sub`) both fail fast.
...
* `OIDC_EXTERNAL_ID_CLAIM` and `OIDC_EMAIL_CLAIM` must not resolve to the same token claim. Cardinal UI refuses to boot otherwise.
