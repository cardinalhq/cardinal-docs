...
- **Dashboards → Add Dashboard → Import JSON** creates a dashboard from it.
...
3. Use that file as a template: substitute service names, thresholds or queries in your own tooling.
4. Load the results back with **Import JSON** or the [API](/ui/dashboards/api).

## File formats Import accepts

**Import JSON** accepts either of these:
...
Import is forgiving. A panel with an unknown `kind`, or one missing `id`, `title` or `queries`, is dropped with a warning instead of failing the whole file. The same goes for an invalid variable or grid cell. The schema is stricter: it rejects unknown fields. So a spec that validates against the schema will always import cleanly.
...
| `category` | no | Where the dashboard is listed on the Dashboards page: `applications`, `infrastructure`, `kubernetes`, `eks`, `ecs`, `rds`, `sqs`, `kafka`, `aws-networking`, `proxmox`, `ceph`, `bind`, `headscale`, `outcomes`. Omitted → *Uncategorized*. **Import JSON** ignores this field and uses the category you pick in the dialog; the API honors it. |
...
| `includeAll` | Offer an **All** option, substituted as `.+`. |
