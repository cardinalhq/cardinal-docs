...
- **Levels by service** shows each service's mix of log levels as a full-width bar, with its total log count and the share of its logs at ERROR or FATAL level (for example, `2.3% err`). Services with the highest error share are listed first. A service with no errors shows `0% err`; its bar is mostly green only because INFO logs are green.
- **Count by service** charts log volume over time, one series per service.
- **Count by level** charts log volume over time, one series per level.

- In **Levels by service**, click a **service row** to open its severity history over time.
