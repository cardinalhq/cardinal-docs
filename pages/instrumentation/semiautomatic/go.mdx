...
This guide describes a simple way to add this that does not depend on any specific vendor's toolkit, and emits the OpenTelemetry signals via OTLP protocol over HTTP.

## Setting up telemetry

For OpenTelemetry, we need to configure various components.  These are propagators for traces, and providers and exporters for all signal types.  By default, any instrumentation that uses the OpenTelemetry Go SDK will use a "no-op" provider, meaning all data will be dropped.  This feature allows setting up the telemetry exporting path after logging is configured and metrics may have been defined, and those will still work.
...
This is handled via environment variables in the common cases, and can be adjusted in code if necessary.

Set `OTEL_EXPORTER_OTLP_ENDPOINT` to the HTTP endpoint of your OpenTelemetry Collector agent.  If you are using CardinalHQ's Managed Collector, this is the "agent" collector, which runs on each node.  You can use the service name, or you can use the "local host" IP address.

Set `OTEL_SERVICE_NAME` to the name of your service.  This is used to set the otel resource `service.name` attribute.

If you are using Kubernetes, a common way to set these is via the deployment manifest.  In the pod template spec, add:
...
Also note that if you are using CardinalHQ's Managed Collector, you may not need to set the service name as it will be obtained automatically when sending to the agent collector.
